...
def privacy_agent(state: State) -> dict:
    ...
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia bảo mật dữ liệu và quyền riêng tư. Phân tích khía cạnh bảo mật và quyền riêng tư dữ liệu:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR, Luật An ninh mạng Việt Nam, quyền riêng tư, rò rỉ dữ liệu (data breach), các hình thức xử phạt về bảo mật thông tin."""
...
def build_graph() -> StateGraph:
    ...
    graph.add_node("check_routing", lambda state: {})
    ...
    graph.add_conditional_edges("check_routing", check_routing)
